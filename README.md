...
- **专为爬虫优化**：底层 `libxml2` 静态库经过极度精简（裁剪了网络、写入器、FTP、HTTP、额外编码等模块），大幅减小编译体积。
- **并发与多线程**：`Document` 与 `SelectedNode` 实现 `Send` 和 `Sync`，原生支持现代多线程爬虫架构。
- **零外部依赖**：无需系统安装 libxml2，源码编译静态链接，极大降低部署成本。
- **移动端友好**：精简编译配置，避免交叉编译地狱。
- **内存安全**：通过生命周期绑定确保节点不会超出文档生命周期。
- **安全封装**：所有 unsafe 代码被隔离在内部模块，公开 API 100% 安全。
- **容错解析**：专为处理真实世界的脏 HTML 设计，默认启用最大容错恢复模式。
